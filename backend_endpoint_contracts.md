...
- `threads.updated_at`: current server timestamp.
...
Notes:
- `thread_id`, `speaker_id`, `position`, and every `persons_involved` item must be integers.
- `quote_text` and `source_url` must be non-empty strings.
- `published_at` must parse as a valid timestamp.
...
Notes:
- `thread_id`, `position`, and every `persons_involved` item must be integers.
